    ...
    { // declara el constructor, si trae el numero de cliente lo busca , si no, trae todos los clientes
        if ($id != 0) {
			...
			try {
				...
				// Select one record from table jos_logicaldoc_configuration where idConfiguration = $id
				// Order it by the ordering field.
				$query = "SELECT idConfiguration, name, username, password, url FROM jos_logicaldoc_configuration WHERE idConfiguration = " . $id;
